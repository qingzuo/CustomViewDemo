...
public class RoundProgressBar extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		switch (style) {
			case STROKE:{
				paint.setStyle(Paint.Style.STROKE);
				canvas.drawArc(oval, 135, 360 * mProgress * 3 / (max * 4), false, paint);  //根据进度画圆弧
				break;
			}
			case FILL:{
	 ...
	 * @return
	 */
	public synchronized int getmTargetProgress() {
	 ...
	 * @param mTargetProgress
	 */
	public synchronized void setmTargetProgress(int mTargetProgress) {
		if(mTargetProgress < 0){
			throw new IllegalArgumentException("mTargetProgress not less than 0");
		}
		if(mTargetProgress > max){
			mTargetProgress = max;
		}
		if(mTargetProgress <= max){
